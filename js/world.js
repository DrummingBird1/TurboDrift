...
function mkSky(THREE, scene) {
  ...
  halo.position.set(450, 280, -550); scene.add(halo);
...
function mkGround(THREE, scene) {
  ...
  const m = new THREE.Mesh(gg, new THREE.MeshStandardMaterial({ color: 0x141f14, roughness: .95 }));
  m.rotation.x = -Math.PI / 2; m.position.y = -.5; m.receiveShadow = true; scene.add(m);
}
...
function mkRoad(THREE, scene) {
  ...
  const rm = new THREE.Mesh(new THREE.ShapeGeometry(sh), new THREE.MeshStandardMaterial({ color: 0x2a2a2a, roughness: .55, metalness: .2 }));
  ...
  for (let i = 0; i < T_N; i += 2) {
    ...
    [1, -1].forEach((side, si) => {
      ...
      bm.position.set(bx, .6, bz); bm.rotation.y = t; bm.castShadow = true; scene.add(bm);
      ...
      if (i % 12 === 0 && si === 0 && lc < 28) {
        ...
        pole.position.set(bx, 4, bz); scene.add(pole);
        const col = i % 24 === 0 ? 0xff6600 : 0x88ccff;
        const pl = new THREE.PointLight(col, 2, 45); pl.position.set(bx, 8, bz); scene.add(pl);
        const glow = new THREE.Mesh(new THREE.SphereGeometry(.25), new THREE.MeshBasicMaterial({ color: col }));
        glow.position.set(bx, 8, bz); scene.add(glow); lc++;
      }
    });
  }
  ...
  [[-aw / 2], [aw / 2]].forEach(([ox]) => {
    ...
    lp.position.set(trackPts[0].x + ox, 5, trackPts[0].z); lp.castShadow = true; scene.add(lp);
    colls.push({ x: trackPts[0].x + ox, z: trackPts[0].z, r: 1.5 });
  });
  const archBar = new THREE.Mesh(new THREE.BoxGeometry(aw, 1.2, 1.2), am);
  archBar.position.set(trackPts[0].x, 10, trackPts[0].z); scene.add(archBar);
}
...
function mkCity(THREE, scene) {
  ...
  [[200, 150, 0xff4400], [-220, -200, 0x00aaff], [0, 350, 0xff00aa]].forEach(([tx, tz, col]) => {
    ...
    top.position.set(tx, h - 1, tz); scene.add(top);
    const antenna = new THREE.Mesh(new THREE.CylinderGeometry(.2, .2, 15), mat);
    antenna.position.set(tx, h + 7.5, tz); scene.add(antenna);
    ...
    gl.position.set(tx, h + 15, tz); scene.add(gl);
  ...
  [trackPts[25], trackPts[60]].forEach((tp, i) => {
    ...
    canopy.position.set(x, 5, z); canopy.castShadow = true; scene.add(canopy);
    colls.push({ x, z, r: 7 }); // covers the whole canopy footprint
    [[-5, -3], [-5, 3], [5, -3], [5, 3]].forEach(([px, pz]) => {
      const post = new THREE.Mesh(new THREE.CylinderGeometry(.2, .2, 5), mat);
      post.position.set(x + px, 2.5, z + pz); scene.add(post);
    });
    [-2.5, 2.5].forEach(px => {
      const pump = new THREE.Mesh(new THREE.BoxGeometry(1, 2.5, .7), new THREE.MeshStandardMaterial({ color: 0xcc0000, metalness: .5 }));
      pump.position.set(x + px, 1.25, z); scene.add(pump);
    });
    ...
    shop.position.set(x, 2, z - 7); shop.castShadow = true; scene.add(shop);
  ...
  for (let i = 0; i < 10; i++) {
    ...
    post.position.set(bx, 3.5, bz); scene.add(post);
    ...
    board.position.set(bx, 8.5, bz); board.rotation.y = t; scene.add(board);
...
function mkProps(THREE, scene) {
  for (let i = 0; i < 250; i++) {
    ...
    leaves.position.set(x, h * .55, z); leaves.scale.y = 1.3; leaves.castShadow = true; scene.add(leaves);
  ...
  for (let i = 0; i < T_N; i += 6) {
    ...
    if (ll < 18) {
      const l = new THREE.PointLight(0xffddaa, 1, 20); l.position.set(lx, 5.2, lz); scene.add(l); ll++;
    }
    const bulb = new THREE.Mesh(new THREE.SphereGeometry(.15), new THREE.MeshBasicMaterial({ color: 0xffddaa }));
    bulb.position.set(lx, 5.2, lz); scene.add(bulb);
  }
  ...
  for (let i = -5; i <= 5; i++) {
    ...
    cone.position.set(cx, .25, cz); scene.add(cone);
    colls.push({ x: cx, z: cz, r: .5 });
  }
}

function mkBoostPads(THREE, scene) {
  // Place 6 boost pads at known indexes along the track
  ...
  for (const idx of positions) {
    ...
    pad.position.set(cx, .08, cz); pad.rotation.y = t; scene.add(pad);
    // chevron arrows on top
...
export function updateAnimated(time, carPos) {
  for (const o of animated) {
    if (o.type === 'cloud') {
      ...
      const c = .15 + Math.sin(time * 3 + o.phase) * .1;
      o.mesh.material.emissiveIntensity = c;
